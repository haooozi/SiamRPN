...
def generate_anchors(total_stride, base_size, scales, ratios, score_size):
    ...
    for ratio in ratios:
        ...
        for scale in scales:
            ...
            count += 1
    #tile堆叠功能，瓷砖的意思 anchor[5,4]->[5,4*19*19]默认维度是第1维，不是第0维
...
def get_exemplar_image(img, bbox, size_z, context_amount, img_mean=None):
    ...
    s_z = np.sqrt(wc_z * hc_z)  #上下文信息
    scale_z = size_z / s_z #缩放因子127/上下文信息(原图得到包含上下文信息的目标框->127)
...
def get_instance_image(img, bbox, size_z, size_x, context_amount, img_mean=None):
    ...
    s_x = s_z * size_x / size_z  #271/127*上下文信息
    #scale_x缩放因子= 271/（271/127*缩放因子）
    instance_img, scale_x = crop_and_pad(img, cx, cy, size_x, s_x, img_mean)
    #w_x,h_x是对应271x271大小图片中目标的宽高
    w_x = w * scale_x     #尺度缩放因子
    h_x = h * scale_x
